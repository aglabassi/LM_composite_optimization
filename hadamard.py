...
for r , kappa in keys:
    ...
    for method in methods:
        x = x0.copy()
        file_name = f'experiments/exphad_{method}_l_{1}_r*={r_true}_r={n}_condn={kappa}_trial_{0}.csv'
        errs = []
        for i in range(T):
...
losses = collect_compute_mean(keys, 1, r_true, False, methods, 'had' )
plot_losses_with_styles(losses, r_true, 1, base_dir, 'Hadamard')
